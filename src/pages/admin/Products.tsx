...
export default function Products() {
  ...
  const deleteMutation = useMutation({
    mutationFn: async (productId: string) => {
      return await apiRequest(`/products/${productId}`, { method: 'DELETE' }) as { action?: 'deleted' | 'deactivated' };
    },
    onSuccess: (result) => {
      queryClient.invalidateQueries({ queryKey: ['admin-products'] });
      queryClient.invalidateQueries({ queryKey: ['admin-products-count'] });
      if (result?.action === 'deactivated') {
        toast.success('Produto desativado porque já está vinculado a pedidos.');
      } else {
              ...
              <CardContent className="p-4">
                ...
                <div className="flex items-center justify-between mt-4">
                  ...
                  <div className="flex gap-2">
                    <Button variant="outline" size="icon" onClick={() => navigate(`/admin/products/edit/${product.id}`)}>
      ...
      <AlertDialog open={!!deletingProduct} onOpenChange={() => setDeletingProduct(null)}>
        <AlertDialogContent>
          <AlertDialogHeader>
            ...
            <AlertDialogDescription>Tem certeza que deseja excluir "{deletingProduct?.name}"?</AlertDialogDescription>
          </AlertDialogHeader>
          <AlertDialogFooter>

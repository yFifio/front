...
serve(async (req) => {
  ...
  try {
    ...
    if (!token) {
      return new Response(
        JSON.stringify({ error: "Token de download não fornecido" }),
    ...
    if (downloadError || !download) {
      ...
      return new Response(
        JSON.stringify({ error: "Link de download inválido ou expirado" }),
    ...
    if (download.orders?.status !== "paid") {
      return new Response(
        JSON.stringify({ error: "Pedido ainda não foi confirmado" }),
    ...
    if (download.download_count >= download.max_downloads) {
      return new Response(
        JSON.stringify({ error: "Número máximo de downloads atingido" }),
    ...
    if (filesError || !files || files.length === 0) {
      ...
      return new Response(
        JSON.stringify({ error: "Nenhum arquivo encontrado para este produto" }),
        { headers: { ...corsHeaders, "Content-Type": "application/json" }, status: 404 }
      );
    }
